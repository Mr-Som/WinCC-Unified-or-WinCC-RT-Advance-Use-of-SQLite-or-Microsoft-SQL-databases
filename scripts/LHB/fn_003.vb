Sub FN_003()
    ...
    Dim test_id, draining_125L, draining_75L, draining_150L, bp_strainer, fp_strainer, bp_chrg_time
    ...
    If HmiRuntime.SmartTags("NRV_125L_OK_BIT") = True Then
        draining_125L = "yes"
    Else
        draining_125L = "no"
    End If

    If HmiRuntime.SmartTags("NRV_75L_OK_BIT") = True Then
        draining_75L = "yes"
    Else
        draining_75L = "no"
    End If

    If HmiRuntime.SmartTags("NRV_150L_OK_BIT") = True Then
        draining_150L = "yes"
    Else
        draining_150L = "no"
    End If

    If HmiRuntime.SmartTags("NRV_BPST_OK_BIT") = True Then
        bp_strainer = "yes"
    Else
        bp_strainer = "no"
    End If

    If HmiRuntime.SmartTags("NRV_FPST_OK_BIT") = True Then
        fp_strainer = "yes"
    Else
        fp_strainer = "no"
    End If

    If HmiRuntime.SmartTags("NRV_ALLOFF_BIT") = True Then
        all_off = "yes"
    Else
        all_off = "no"
    End If
    ...
    bp_chrg_time = HmiRuntime.SmartTags("BPCH_TIME")
    ...
    Else
        ' If not exists → INSERT
            ...
            "id, draining_of_125l, draining_of_75l, draining_of_150l, bp_strainer, fp_strainer, all_off, bp_chrg_time, front_pwr_car_fp, rear_pwr_car_fp" & _
            ") VALUES (" & _
            test_id & ",'" & draining_125L & "','" & draining_75L & "','" & draining_150L & "','" & bp_strainer & "','" & fp_strainer & "','" & all_off & "'," & bp_chrg_time & ",'" & front_fp & "','" & rear_fp & "')"
    End If
    ...
    If Err.Number <> 0 Then
        strError = "Error in FN_003: " & Err.Description
        timeStamp = Now
